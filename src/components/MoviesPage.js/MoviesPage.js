...
function MoviesPage() {
  ...
  const fetchMovies = async (pageNumber, setNumberOfPages, moviesLimit) => {
    let result = await fetch(`${baseUrl}/movies/allmovies?page=${pageNumber}&limit=${moviesLimit}&isSeries=Movie`);
  ...
  return (
    <div className="moviespagecontainer">
      ...
      {isLoading ? (
        ...
        <div className="moviespageholder" style={{ boxShadow: isDark ? '' : 'none' }}>
          <h1 className="moviespageheader">#movies</h1>
